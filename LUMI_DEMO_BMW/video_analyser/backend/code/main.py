...
async def trigger_grasp_action(number):
    url = "http://127.0.0.1:12123/grasp"
    try: 
        async with aiohttp.ClientSession() as session:
            await session.get(url, params={"number": number})
            logger.info(f"已触发抓取指令，物品编号: {number}")
    except Exception as e:
        logger.error(f"抓取指令发送失败: {e}")

async def save_uploaded_images(request: ArkChatRequest) -> List[str]:
    ...
    for idx, part in enumerate(content):
        ...
        if image_url is None or not getattr(image_url, "url", None):
            continue
        ...
        try:
            if url.startswith("data:image/"):
                ...
                ext = header.split(";", 1)[0].split("/", 1)[1].lower()
                if ext == "jpeg":
                    ext = "jpg"
                ...
                logger.warning("unsupported image_url: %s", url[:64])
                continue
        except Exception:
            logger.exception("failed to decode image part %s", idx)
            continue

        filename = f"{int(time.time() * 1000)}_{uuid.uuid4().hex}_{idx}.{ext}"
    ...
    if saved:
        logger.info("saved %d image(s) to %s", len(saved), images_dir)
    return saved
...
async def default_model_calling(
    ...
    if isinstance(request.messages[-1].content, list):
        asyncio.create_task(save_uploaded_images(request))
    ...
    message = ""
    grasp_triggered = False # 防止一句话里重复触发
    drink_map = {"可乐": "2", "芬达": "1", "雪碧": "3"}  
    ...
    async for resp in create_bot_audio_responses(tts_stream_output, request):
        ...
        # 2. [新增] 意图检测逻辑
        if not grasp_triggered:
            # 使用正则匹配 "正在为您拿(可乐|芬达|雪碧)"
            match = re.search(r"正在为您拿(可乐|芬达|雪碧)", message)
            # print("匹配", match)
            if match:
                drink_name = match.group(1)
                if drink_name in drink_map:
                    # 使用 asyncio.create_task 进行“射后不理”的异步调用，不卡顿语音
                    asyncio.create_task(trigger_grasp_action(drink_map[drink_name]))
                    grasp_triggered = True # 标记已触发，避免重复发送
        yield resp
...
if __name__ == "__main__":
    ...
    launch_serve(
        package_path="main",
        port=int(port) if port else 8887,
